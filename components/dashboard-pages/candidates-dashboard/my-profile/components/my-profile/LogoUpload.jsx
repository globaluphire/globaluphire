...
const LogoUpload = () => {
    ...
    const [cloudPath, setCloudPath] = useState("https://gytbjlatclocthcyugmq.supabase.co/storage/v1/object/public/applications/dp/");
            ...
            if (userID) {
                ...
                if (customer) {
                    ...
                    if (customer[0].dp != "" && customer[0].dp != null) {
                        setLogoFilename(cloudPath + encodeURIComponent(customer[0].dp));
    ...
    const handleFileLogoChange = async (event) => {
        ...
        if (selectedFile) {
            ...
            const { data: fileUploadSuccess, error: fileUploadError } = await supabase
                .storage
                .from('applications')
                .upload('dp/' + fileTimestamp + '-' + selectedFile.name, selectedFile, file);
            if (fileUploadError) {
                ...
                const { data: docURL, error: docURLError } = supabase
                    .storage
                    .from('applications')
                    .getPublicUrl('dp/' + fileTimestamp + '-' + selectedFile.name)
                if (docURLError) {

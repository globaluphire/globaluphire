...
const UserDocuments = ({ applicantData }) => {
    ...
    const [sendDocumentsForSigningLoading, setSendDocumentsForSigningLoading] =
        useState(false);
    ...
    const sendDocumentsForSigning = async (user) => {
        ...
        try {
            ...
            const applicantEmail =
                applicantForm.current.querySelector("#applicantEmail").value;
            const applicantName =
                applicantForm.current.querySelector("#applicantName").value;
    ...
    const fetchTemplates = async (applicantData) => {
        ...
        try {
            const token = await getAccessToken(user.email);
            const response = await fetch("/api/ds/templates", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            if (response.ok) {
                ...
                const hasNotSent = data?.data?.every(
                    (item) => item?.envelope?.status === "not sent"
                );
                let newStatus = "";
                if (hasNotSent) {
                    newStatus = "Not Sent";
                }
                const hasSent = data?.data?.some(
                    (item) => item?.envelope?.status === "sent"
                );
                const hasDelivered = data?.data?.some(
                    (item) => item?.envelope?.status === "delivered"
                );
                const hasCompleted = data?.data?.some(
                    (item) => item?.envelope?.status === "completed"
                );
                if (hasDelivered || hasSent || hasCompleted) {
                    const allItemsCompleted = data?.data.every(
                        (item) => item?.envelope?.status === "completed"
                    );
                    newStatus = allItemsCompleted
                        ? "Signed"
                        : hasDelivered
                        ? "Read"
                        : "Sent";
        ...
        <form className="default-form">
            <div className="row">
                <div className="col-6">
                    ...
                    >
                        <h5 className="mb-3" style={{ display: "flex" }}>
                            Documents
                        ...
                        <span style={{ cursor: "pointer" }}>
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    handleAllTemplatesSelect(e.target.checked);
                    ...
                    <Form ref={applicantForm} className="mt-5">
                        <Form.Group className="form-group">
                            <Form.Label htmlFor="applicantEmail">
                                Email
                            </Form.Label>
                            <Form.Control
                ...
                <div className="col-6">
                    ...
                    <ListGroup
                        ...
                        style={{
                            ...
                            alignItems: "center",
                            justifyContent: `${
                                imgData === null ? "center" : "flex-start"
                            }`,
                        }}
                    >

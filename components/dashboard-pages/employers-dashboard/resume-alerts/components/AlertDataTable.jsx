...
const AlertDataTable = () => {
    ...
    const fetchPost = async (newKeyword, pageNo) => {
        ...
        if (newKeyword != null) {
            ...
            const countTotalRecords = await supabase
                .from("notification")
                .select("*", { count: "exact", head: true })
                // .eq('user_id', user.id)
                .is("deleted", null);
            const totalRecords = countTotalRecords.count;

            const recordPerPage = rpp;
            ...
            if (totalPages) {
                ...
                setArrPages(arrPage);
                let start_limit = parseInt(
                    parseInt(pageNo - 1) * parseInt(rpp)
                );
                if (pageNo < 1) {
                    start_limit = parseInt(parseInt(pageNo) * parseInt(rpp));
                }
                const end_limit = parseInt(start_limit) + parseInt(rpp);
            ...
            <div>
                <div className="search-box-one">
                    <form method="post" action="blog.html">
                        <div
                            className="form-group pull-left mb-3"
                                    ...
                                    <td>
                                        <div
                                            dangerouslySetInnerHTML={{
                                                __html: candidate.notification_text,
                                            }}
                                        ></div>
                                    </td>
                                    <td>
                ...
                {isLoading === false && userData.length === 0 && (
                    ...
                    >
                        <center>No resume alert yet!</center>
                    </p>
                )}
                        ...
                        <ul>
                            ...
                            {arrPages.map((item) => {
                                if (arrPages.length > 6) {
                                    const nextThreePages = item - 4;
                                    const prevThreePages = item + 4;

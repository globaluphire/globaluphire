...
const TopCardBlock = () => {
    ...
    const [totalWithdrawedApplications, setTotalWithdrawedApplications] =
        useState(0);
    const [totalRejectedApplications, setTotalRejectedApplications] =
        useState(0);
    ...
    const fetchDashboardData = async () => {
        ...
        if (facility) {
            // fetch data for Jobs
            const countTotalPostedJobs = await supabase
                .from("jobs")
            ...
            const countTotalPublishedJobs = await supabase
                .from("jobs")
                .select("*", { count: "exact", head: true })
                .eq("facility_name", facility)
                // .eq('user_id', user.id)
            ...
            const countTotalUnpublishedJobs = await supabase
                .from("jobs")
                .select("*", { count: "exact", head: true })
                .eq("facility_name", facility)
                // .eq('user_id', user.id)
             ...
             */
            const countTotalApplications = await supabase
             ...
             */
            const countTotalNewApplications = await supabase
                .from("applicants_view")
                .select("*", { count: "exact", head: true })
                .eq("facility_name", facility)
                .eq("status", "New");
             ...
             * fetch data for Applications
             */
            const countTotalHiredApplications = await supabase
                .from("applicants_view")
                .select("*", { count: "exact", head: true })
                .eq("facility_name", facility)
                .eq("status", "Hired");
             ...
             * fetch data for Applications
             */
            const countTotalWithrawedApplications = await supabase
                .from("applicants_view")
                .select("*", { count: "exact", head: true })
                .eq("facility_name", facility)
                .eq("status", "Withdraw");
            ...
            // .is('deleted', null);
            // console.log("countTotalApplications", countTotalApplications);
            setTotalWithdrawedApplications(
                countTotalWithrawedApplications.count
            );

            // fetch data for Applications
            const countTotalRejectedApplications = await supabase
                .from("applicants_view")
                .select("*", { count: "exact", head: true })
                .eq("facility_name", facility)
                .eq("status", "Rejection");
    ...
    return (
        <>
            ...
            {isLoading === false &&
                applicationCardContent.map((item) => (
                    <div
                        className="ui-block col-xl-3 col-lg-6 col-md-6 col-sm-12"
                        key={item.id}
                    >

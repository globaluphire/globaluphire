...
const Applicants = () => {
    ...
    const [cloudPath, setCloudPath] = useState(
        "https://ntvvfviunslmhxwiavbe.supabase.co/storage/v1/object/public/applications/cv/"
    );
    ...
    const fetchDashboardData = async () => {
        setIsLoading(true);
        const dataRecentApplicants = await supabase
        ...
        if (dataRecentApplicants) {
            ...
            const unique = dataRecentApplicants.data.filter(
                (obj, index) =>
                    dataRecentApplicants.data.findIndex(
                        (item) => item.user_id === obj.user_id
                    ) === index
            );
            const arrData = [];
            for (const item of unique) {
                ...
                if (fetchUser) {
                    ...
                    if (
                        fetchUser.data &&
                        fetchUser.data.length > 0 &&
                        fetchUser.data[0].user_photo != null
                    ) {
                        photo_url = cloudPath + fetchUser.data[0].user_photo;
                    } else if (
                        fetchUser.data &&
                        fetchUser.data.length > 0 &&
                        fetchUser.data[0].photo_url != null
                    ) {
                    ...
                    >
                        <div className="inner-box">
                            <div className="content">
                                ...
                                <h4 className="name">
                                    <Link
                                        href={`/candidate-details/${candidate.user_id}`}
                                    >
                                        {candidate.name}
                                    </Link>
                                </h4>

const DescriptionConent = () => {
    return (
        <div className="content">
            <h4>Details</h4>
            <p>
                Do you want to become a UI/UX designer but you don&apos;t know
                where to start? This course will allow you to develop your user
                interface design skills and you can add UI designer to your CV
                and start getting clients for your skills.
            </p>
            <p>
                Hi everyone. I&apos;m Arash and I&apos;m a UI/UX designer. In
                this course, I will help you learn and master Figma app
                comprehensively from scratch. Figma is an innovative and
                brilliant tool for User Interface design. It&apos;s used by
                everyone from entrepreneurs and start-ups to Apple, Airbnb,
                Facebook, etc.
            </p>
            <h4>Material and care</h4>
            <ul className="list-style-two">
                <li>Main: 100% Cotton</li>
                <li>One size fits all</li>
                <li>Soft twill</li>
                <li>Imported</li>
                <li>Ribbed, diagonal pattern</li>
                <li>Product color: Dark greenish</li>
            </ul>
        </div>
    );
};

export default DescriptionConent;
